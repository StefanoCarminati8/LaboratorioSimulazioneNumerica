...
//g(r)
    for(int s=igofr; s<igofr+nbins; ++s){
      r=s*bin_size;
      dV=(4/3)*M_PI*(pow(r+bin_size, 3)-pow(r,3));
      gdir = (1/(rho*dV*(double)npart))*(blk_av[s]/blk_norm);
      glob_av[s] += gdir;
      glob_av2[s] += gdir * gdir;
      err_gdir=Error(glob_av[s], glob_av2[s], iblk);
      Gofr << setw(wd) << iblk <<  setw(wd) << r << setw(wd) << gdir << endl;
      if(iblk == nblk){
        Gave << setw(wd) << iblk <<  setw(wd) << r << setw(wd) << glob_av[s]/(double)iblk << setw(wd) << err_gdir << endl;
      }
    }
...
{
  ofstream Epot, Press;
  Epot.open("epotistant.0", ios::app);
  Press.open("pressistant.0", ios::app);

  double epot = walker[iv]/(double)npart + vtail;
  double press = rho * temp + (walker[iw] + ptail * (double)npart) / vol;

  Epot << setprecision(12) << epot << endl;
  Press << setprecision(12) << press << endl;
