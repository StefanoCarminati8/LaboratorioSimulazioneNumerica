...
void Medie(int iblk){
  ...
  //g(r)  distribuzione radiale
  for(int k=igofr; k<igofr + nbins; k++){
    r = (k-3)*binSize;
    deltaV = (4/3)*M_PI*(pow(r+binSize, 3)-pow(r,3));
    stima_gr = (1/(rho*deltaV*(double)npart))*(blk_av[k]/blk_norm);
    glob_av[k] += stima_gr;
    glob_av2[k] += stima_gr * stima_gr;
    err_gr = Error(glob_av[k], glob_av2[k], iblk);
    gr << " " << setprecision(12) << r << " " << setprecision(12) << stima_gr << endl;
    if(iblk == nblocchi){
      gmedia << " " << setprecision(12) << r << " " << setprecision(12) << glob_av[k]/(double)iblk << " " << setprecision(12) << err_gr << endl;
    }
  }
